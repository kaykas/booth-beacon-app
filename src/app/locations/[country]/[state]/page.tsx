...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `Photo Booths in ${state}, ${country} | Booth Beacon`,
    description: `Discover analog photo booths in ${state}, ${country}. Find locations, contact details, and more.`,
  };
}
...
export default async function StatePage({ params, searchParams }: PageProps) {
  ...
  const { booths, totalCount, hasMore } = await getLocationBooths(country, state, undefined, {
    limit,
    offset,
  });
    ...
    <>
      ...
      <div className="mb-8">
        ...
        <p className="max-w-2xl text-lg text-muted-foreground">
          {totalCount.toLocaleString()} photo booths found in {state}, {country}.{' '}
          {cities.length > 0 ? `Explore by city or browse all booths below.` : `Browse all booths below.`}
        </p>
      </div>
