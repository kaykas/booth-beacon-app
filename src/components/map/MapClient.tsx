...
export function MapClient({ nearme, city }: MapClientProps) {
  ...
  // Enable auto-center once we have user location
  useEffect(() => {
  ...
  // Update filteredBooths with async handling for distance sorting
  useEffect(() => {
        ...
        <div className="flex-1 relative">
          {loading ? (
            ...
            <BoothMap
              ...
              onCenterComplete={() => {
                setAutoCenter(false);
                // Enable distance sorting after map centers (for Near Me feature)
                if (nearme === 'true' && userLocation) {
                  setSortByDistance(true);
                }
              }}
