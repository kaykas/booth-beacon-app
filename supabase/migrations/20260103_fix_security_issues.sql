...
-- 2. Enable RLS on tables without it (spatial_ref_sys, crawl_jobs, crawl_raw_content)
--
-- WARN-level fixes:
-- 3. Add SET search_path = '' to all functions
-- 4. Document postgis extension in public schema (acceptable for PostGIS)
...
-- Note: This is a PostGIS system table. Enabling RLS with permissive policy
-- to maintain compatibility while satisfying linter.
DO $$
BEGIN
  IF EXISTS (
    ...
    -- Allow public read access (standard for PostGIS reference data)
    DROP POLICY IF EXISTS "Allow public read access to spatial_ref_sys" ON spatial_ref_sys;
    CREATE POLICY "Allow public read access to spatial_ref_sys"
      ON spatial_ref_sys FOR SELECT
      USING (true);

    -- Only service_role can modify (this is reference data)
    DROP POLICY IF EXISTS "Allow service_role to manage spatial_ref_sys" ON spatial_ref_sys;
    CREATE POLICY "Allow service_role to manage spatial_ref_sys"
      ON spatial_ref_sys FOR ALL
...
BEGIN
  ...
  RAISE NOTICE '   - Enabled RLS on 3 tables (spatial_ref_sys, crawl_jobs, crawl_raw_content)';
