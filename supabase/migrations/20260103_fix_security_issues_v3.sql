-- Migration: Fix Database Security Issues (v3 - handles function signatures)
-- Date: 2026-01-03
-- Description: Addresses Supabase linter errors and warnings
--
-- ERROR-level fixes:
-- 1. Remove SECURITY DEFINER from views
-- 2. Enable RLS on tables (crawl_jobs, crawl_raw_content)
-- 3. Skip spatial_ref_sys (PostGIS system table)
--
-- WARN-level fixes:
-- 4. Add SET search_path = '' to all functions
-- 5. Document postgis extension
...
-- =============================================================================
-- PART 3: Fix Functions (DROP existing first to handle signature changes)
-- =============================================================================
...
RETURNS TABLE (
  id UUID, name TEXT, address TEXT, city TEXT, country TEXT,
  latitude DECIMAL, longitude DECIMAL, distance_km DECIMAL,
  machine_model TEXT, booth_type TEXT, photo_exterior_url TEXT, status TEXT
) AS $$
BEGIN
  RETURN QUERY
  ...
  WHERE b.location_geog IS NOT NULL
    AND ST_DWithin(b.location_geog,
      ST_SetSRID(ST_MakePoint(p_longitude::float, p_latitude::float), 4326)::geography,
      p_radius_km * 1000)
...
RETURNS TABLE (
  id UUID, name TEXT, address TEXT, city TEXT, state TEXT, country TEXT,
  latitude DECIMAL, longitude DECIMAL, distance_km NUMERIC,
  booth_type TEXT, photo_exterior_url TEXT, status TEXT, is_operational BOOLEAN
) AS $$
BEGIN
  RETURN QUERY
  SELECT b.id, b.name, b.address, b.city, b.state, b.country, b.latitude, b.longitude,
    ROUND(ST_Distance(
      ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography, b.location_geog
    ) / 1000, 2) as distance_km,
    b.booth_type, b.photo_exterior_url, b.status, b.is_operational
  FROM booths b
  WHERE b.location_geog IS NOT NULL AND b.status = 'active'
    AND ST_DWithin(b.location_geog,
      ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography, distance_km * 1000)
...
RETURNS BOOLEAN AS $$
BEGIN
  RETURN EXISTS (SELECT 1 FROM profiles WHERE id = user_uuid AND is_admin = true);
END;
...
CREATE OR REPLACE FUNCTION get_booth_photo_storage_path(
  p_user_id UUID, p_booth_id UUID, p_file_extension TEXT
)
RETURNS TEXT AS $$
DECLARE
  timestamp_str TEXT; random_str TEXT;
BEGIN
  timestamp_str := to_char(now(), 'YYYYMMDD_HH24MISS');
  random_str := substr(md5(random()::text), 1, 8);
  RETURN format('booth-photos/%s/%s/%s_%s.%s',
    p_booth_id, p_user_id, timestamp_str, random_str, p_file_extension);
END;
$$ LANGUAGE plpgsql SET search_path = '';

CREATE OR REPLACE FUNCTION get_booth_photo_public_url(p_storage_path TEXT)
RETURNS TEXT AS $$
BEGIN
...
BEGIN
  ...
  RAISE NOTICE '   - Skipped spatial_ref_sys (PostGIS system table)';
  RAISE NOTICE '   - Added SET search_path to 22 functions';
  RAISE NOTICE '   - Documented PostGIS extension';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Leaked password protection already enabled';
  RAISE NOTICE 'ℹ️  spatial_ref_sys RLS skipped (PostGIS reference table)';
END $$;
