...
// Price patterns - matches $3, €2, £5, $2-5, etc.
const COST_PATTERNS = [
  /(?:costs?|price[ds]?|charges?|fees?)\s*:?\s*([€$£¥]\s*\d+(?:\.\d{2})?(?:\s*-\s*[€$£¥]?\s*\d+(?:\.\d{2})?)?)/i,
  /([€$£¥]\s*\d+(?:\.\d{2})?(?:\s*-\s*[€$£¥]?\s*\d+(?:\.\d{2})?)?)\s+(?:per|for)\s+(?:strip|session|photos?)/i,
  /([€$£¥]\s*\d+(?:\.\d{2})?(?:\s*-\s*[€$£¥]?\s*\d+(?:\.\d{2})?)?)\s+for\s+\d+\s+photos?/i,
  /\bfree\b/i,
];

// Hours patterns - matches various time formats
const HOURS_PATTERNS = [
  /(?:open|hours?|operating)\s*:?\s*(\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?\s*-\s*\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?)/i,
  /(\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?\s*-\s*\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?)\s+(?:daily|weekdays?|weekends?)/i,
  /(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday|mon|tue|wed|thu|fri|sat|sun)[:\s]+(\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?\s*-\s*\d{1,2}(?::\d{2})?\s*(?:am|pm|AM|PM)?)/i,
  /\b24\/7\b/i,
  /\b24\s*hours?\b/i,
];

function extractBoothType(text: string): string | undefined {
...
function extractHours(text: string): string | undefined {
  for (const pattern of HOURS_PATTERNS) {
    ...
    if (match) {
      // Return the full match or the captured group
      const hours = match[1] || match[0];
      // Clean up whitespace
      return hours.trim().replace(/\s+/g, ' ');
    }
  }
...
function analyzeBooth(booth: Booth): ExtractionResult {
  ...
  // Extract booth_type if missing
  if (!booth.booth_type && combinedText) {
    const extractedType = extractBoothType(combinedText);
    if (extractedType) {
  ...
  // Extract cost if missing
  if (!booth.cost && combinedText) {
    const extractedCost = extractCost(combinedText);
    if (extractedCost) {
      result.cost = extractedCost;
    }
  }

  // Extract hours if missing
  if (!booth.hours && combinedText) {
    const extractedHours = extractHours(combinedText);
    if (extractedHours) {
...
async function getBeforeStats() {
  const { data: booths, error } = await supabase
    .from('booths')
    .select('booth_type, photo_type, cost, hours');
  ...
  return {
    ...
    cost_missing: booths.filter((b) => !b.cost).length,
    hours_missing: booths.filter((b) => !b.hours).length,
  };
}

async function enrichBoothData() {
  console.log('='.repeat(80));
  console.log('BOOTH DATA ENRICHMENT SCRIPT');
  console.log('='.repeat(80));
  console.log('');
  ...
  console.log(`  cost missing: ${beforeStats.cost_missing} (${((beforeStats.cost_missing / beforeStats.total) * 100).toFixed(1)}%)`);
  console.log(`  hours missing: ${beforeStats.hours_missing} (${((beforeStats.hours_missing / beforeStats.total) * 100).toFixed(1)}%)`);
  console.log('');
  ...
  const { data: booths, error } = await supabase
    .from('booths')
    .select('id, name, description, machine_model, historical_notes, booth_type, photo_type, cost, hours, address')
    .order('name');
  ...
  console.log('🔬 Analyzing booths and extracting data...');
  ...
  const costCount = results.filter((r) => r.cost).length;
  const hoursCount = results.filter((r) => r.hours).length;

  console.log('EXTRACTION SUMMARY:');
  console.log(`  booth_type: ${boothTypeCount} extracted`);
  console.log(`  photo_type: ${photoTypeCount} extracted`);
  console.log(`  cost: ${costCount} extracted`);
  console.log(`  hours: ${hoursCount} extracted`);
  console.log('');
  ...
  if (results.length > 0) {
    ...
    for (let i = 0; i < results.length; i += BATCH_SIZE) {
      ...
      for (const result of batch) {
        ...
        if (result.cost) updateData.cost = result.cost;
        if (result.hours) updateData.hours = result.hours;
  ...
  console.log(`  cost missing: ${afterStats.cost_missing} (${((afterStats.cost_missing / afterStats.total) * 100).toFixed(1)}%)`);
  console.log(`  hours missing: ${afterStats.hours_missing} (${((afterStats.hours_missing / afterStats.total) * 100).toFixed(1)}%)`);
  console.log('');
  ...
  console.log(`cost: ${beforeStats.cost_missing - afterStats.cost_missing} fields populated`);
  console.log(`hours: ${beforeStats.hours_missing - afterStats.hours_missing} fields populated`);
  console.log('='.repeat(80));
}

enrichBoothData().catch(console.error);
