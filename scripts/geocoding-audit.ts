...
// Output files
const REPORT_FILE = path.join(__dirname, '../geocoding-audit-report.json');
const CSV_FILE = path.join(__dirname, '../affected-booths.csv');
 ...
 */
async function fetchAllBooths(): Promise<Booth[]> {
  ...
  try {
    ...
    console.error('Error fetching booths:', (error as Error).message);
    throw error;
  }
}
 ...
 */
function analyzeBoothProblems(booth: Booth): Problem[] {
 ...
 */
function findDuplicateCoordinates(booths: Booth[]): Record<string, Booth[]> {
  const coordMap: Record<string, Booth[]> = {};
  const duplicates: Record<string, Booth[]> = {};
 ...
 */
function classifySeverity(problems: Problem[]): string {
...
async function generateAuditReport() {
  ...
  try {
    ...
    // Analyze each booth
    const boothsWithProblems: BoothWithProblems[] = [];
    const problemsByCategory: Record<string, number> = {};
    const affectedBoothIds = new Set<string>();
